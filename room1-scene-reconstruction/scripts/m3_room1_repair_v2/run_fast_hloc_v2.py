...
SPARSE_ATTEMPT = BASE / "sparse_fast_v2"
...
def prepare() -> tuple[list[Path], Path, list[Path]]:
    ...
    for guarded in [HLOC, SPARSE_ATTEMPT, SELECTED_SPARSE]:
        if guarded.exists():
            raise RuntimeError(f"refusing existing output: {guarded}")
    if not IMAGES.exists():
        IMAGES.mkdir()
    existing_names = sorted(path.name for path in IMAGES.glob("frame_*.jpg"))
    expected_names = [path.name for path in paths]
    if existing_names and existing_names != expected_names:
        raise RuntimeError("existing image set differs from strict keyframes")
    HLOC.mkdir()
    for source in paths:
